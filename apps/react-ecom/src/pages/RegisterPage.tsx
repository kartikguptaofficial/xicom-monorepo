...
import { registerUserController } from '../services/auth.service';
...
export default function RegisterPage() {
  ...
  const { isPending, data, mutate: registerUserHander } = useMutation({
    mutationFn: async () => {
      ...
      const loginData: any = await registerUserController({ email, password, name, phone });
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" action="#">
              ...
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                Already a user? <a href="/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Login Now</a>
              </p>
            </form>
          </div>
        </div>
